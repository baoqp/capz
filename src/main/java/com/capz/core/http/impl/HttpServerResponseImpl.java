...
import com.capz.core.http.HttpHeaders;
...
public class HttpServerResponseImpl implements HttpServerResponse {
    ...
    private final CapzHttpHeaders headers;
    ...
    HttpServerResponseImpl(final CapzInternal capz, Http1xServerConnection conn, HttpRequest request) {
        this.capz = capz;
        this.conn = conn;
        this.version = request.getProtocolVersion();
        this.headers = new CapzHttpHeaders();
        this.status = HttpResponseStatus.OK;
        this.keepAlive = true; // TODO
    ...
    public HttpServerResponse putHeader(CharSequence name, CharSequence value) {
        synchronized (conn) {
            checkValid();
            headers.set(name.toString(), value.toString());
            return this;
        }
    }
    ...
    public HttpServerResponse putHeader(CharSequence name, Iterable<CharSequence> values) {
        synchronized (conn) {
            ...
            for(CharSequence value : values) {
                headers.set(name_, value.toString());
            }
            return this;
        }
    }
    ...
    public HttpServerResponseImpl putTrailer(String key, String value) {
        synchronized (conn) {
            checkValid();
            List<String> trailer = trailers().get(key);

            if (trailer == null) {
                trailer = new ArrayList<>();
                trailers().put(key, trailer);
            }
            trailer.add(value);
            return this;
        }
    ...
    private void doSendFile(String filename, long offset, long length, Handler<AsyncResult<Void>> resultHandler) {
        synchronized (conn) {
            ...
            checkValid();
            File file = new File(filename); // TOOD  vertx.resolveFile(filename);

            if (!file.exists()) {
                if (resultHandler != null) {
                    AbstractContext ctx = capz.getOrCreateContext();
        ...
        synchronized (conn) {
            if (!closed) {
                ...
                if (!written && exceptionHandler != null) {
                    conn.getContext().runOnContext(v -> exceptionHandler.handle(new CapzException("Connection was closed")));
                }
                if (endHandler != null) {
